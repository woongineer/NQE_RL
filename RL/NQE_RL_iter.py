...
def plot_nqe_loss(NQE_losses, iter):
    ...
    plt.xticks(range(len(NQE_losses)))
...
def plot_policy_loss(policy_losses, iter):
    ...
    plt.plot(policy_losses_values, color='orange',
             label='Policy Loss')
    plt.xticks(range(len(policy_losses_values)))
...
def plot_comparison(loss_none, loss_NQE, loss_NQE_RL,
    ...
    plt.xticks(range(len(loss_none)))
    plt.title('QCNN')
    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    plt.legend()
...
if __name__ == "__main__":
    ...
    for iter in range(iterations):
        print(f"Starting iteration {iter + 1}/{iterations}")
        # Step 1: Train NQE
        if iter == 0:
    ...
    prediction_with_NQE = [QCNN_classifier(weight_with_NQE, x, "NQE", final_NQE_model) for x in X_test]
