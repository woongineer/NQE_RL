...
def circuit_training(QCNN_lr, QCNN_steps, batch_sz, X_train,
                     Y_train, scheme, NQE_model=None, action_seq=None):
    weights = np.random.random(30, requires_grad=True)
    opt = qml.NesterovMomentumOptimizer(stepsize=QCNN_lr)
    loss_history = []
    for it in range(QCNN_steps):
        batch_index = np.random.randint(0, len(X_train), (batch_sz,))
...
def cost(weights, X_batch, Y_batch, scheme,
         NQE_model=None, action_seq=None):
    preds = []
    for x in X_batch:
        if scheme == 'NQE_RL':
...
@qml.qnode(dev)
def QCNN_classifier(params, x, scheme, NQE_model=None, action_seq=None):
    if scheme == 'NQE_RL':
        statepreparation(x, scheme, NQE_model, action_seq)
    elif scheme == 'NQE':
...
def statepreparation(x, scheme, NQE_model=None, action_seq=None):
    if scheme is None:
        ...
        x = NQE_model.linear_relu_stack1(torch.tensor(x, dtype=torch.float32))
        x = x.detach().numpy()
        quantum_embedding_rl(x, action_seq)
