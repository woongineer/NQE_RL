...
class ActionEmbedding(nn.Module):
    ...
    def forward(self, action):
        return self.embedding(torch.tensor(action))
...
class RepresentationNetwork(nn.Module):
    ...
    """
    def __init__(self, action_embedding, embedding_size, lstm_size, latent_size):
        super().__init__()
        # for gate sequence
        self.action_embedding = action_embedding
        self.lstm = nn.LSTM(embedding_size, lstm_size, batch_first=True)

        self.fc = nn.Linear(lstm_size, latent_size)
...
class Networks(nn.Module):
    ...
    def _value_transform(self, value_support):
        epsilon = 0.001
        value = softmax(value_support, dim=-1)
...
class MCTS:
    ...
    def expand_root(self, node, actions, network, observation):
        ...
        softmax_policy = softmax(torch.squeeze(policy_logits), dim=-1)
...
class QASEnv:
    ...
    def step(self, action):
        ...
        reward = get_fidelity(self.actions, self.X1, self.X2, self.Y)
