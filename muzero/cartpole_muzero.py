    ...
    def make_target(self, state_index, num_unroll_steps, td_steps):
        ...
        for current_index in range(state_index,
            ...
            if bootstrap_index < len(self.root_values):
                value = self.root_values[bootstrap_index][0] * (
            ...
            if current_index < len(
                    self.root_values):  # current_index is within the episode,
                ...
                targets.append(
                    (0, last_reward, np.array([1.0 / num_actions for _ in range(
                        num_actions)])))  # assign value 0 and uniform policy
                actions.append(
                    np.random.choice(num_actions))  # assign a random action
...
class RecurrentModel(nn.Module):
    ...
    def __init__(self, dynamic_network, reward_network, value_network,
                 policy_network):
...
class Networks(nn.Module):
    ...
    def __init__(self, representation_network, value_network, policy_network,
                 dynamic_network, reward_network, max_value):
        super().__init__()
        self.train_steps = 0
        self.action_size = 2
        self.representation_network = representation_network
    ...
    def initial_inference(self, state):
        ...
        return self._value_transform(
            value), 0, policy_logits, hidden_representation

    def recurrent_inference(self, hidden_state, action):
        hidden_state_with_action = self._hidden_state_with_action(hidden_state,
                                                                  action)
        hidden_representation, reward, value, policy_logits = self.recurrent_model(
            hidden_state_with_action)
        return self._value_transform(value), self._reward_transform(
            reward), policy_logits, hidden_representation
    ...
    def _hidden_state_with_action(self, hidden_state, action):
        ...
        hidden_state_with_action = torch.concat(
            (hidden_state,
             torch.tensor(self._action_to_one_hot(action, self.action_size))[
                 0]), axis=0)
    ...
    def _action_to_one_hot(self, action, action_space_size):
        ...
        """
        return np.array([1 if i == action else 0 for i in
                         range(action_space_size)]).reshape(1, -1)
...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        for actions_batch, targets_batch in zip(actions_batch,
                                                targets_recurrent_batch):
            ...
            actions_batch_onehot = F.one_hot(torch.tensor(actions_batch),
                                             num_classes=network.action_size).float()
            state_with_action = torch.cat(
                (hidden_representation, actions_batch_onehot), dim=1)
            hidden_representation, rewards, values, policy_logits = network.recurrent_model(
                state_with_action)
            ...
            # gradient scaling
            gradient_loss_step = scale_gradient(loss_step, (
                        1 / config['num_unroll_steps']))
...
class MCTS():
    ...
    def run_mcts(self, config, root, network, min_max_stats):
        ...
        for i in range(config['num_simulations']):
            ...
            value = self.expand_node(node, list(
                range(config['action_space_size'])), network,
                                     parent.hidden_representation, action)
    ...
    def select_action(self, config, node, test=False):
        ...
        """
        visit_counts = [
            (child.visit_count, action) for action, child in
            node.children.items()
        ]
        if not test:
    ...
    def expand_root(self, node, actions, network, current_state):
        ...
        transformed_value, reward, policy_logits, hidden_representation = network.initial_inference(
            observation)
        ...
        softmax_policy = torch.nn.functional.softmax(
            torch.squeeze(policy_logits))
    ...
    def expand_node(self, node, actions, network, parent_state, parent_action):
        ...
        transformed_value, reward, policy_logits, hidden_representation = network.recurrent_inference(
...
def self_play(env, config, replay_buffer, network):
    ...
    for i in range(
            config['num_epochs']):  # Number of Steps of train/play alternations
