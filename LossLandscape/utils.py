...
def compute_QNTK(nn_model, random_circuit, input_data):
    nn_model.eval()
    params = torch.nn.utils.parameters_to_vector(nn_model.parameters()).detach()

    def qntk_single_sample(params, single_input):
        torch.nn.utils.vector_to_parameters(params, nn_model.parameters())
        output = nn_model(single_input.unsqueeze(0))
        return output

    jacobian = torch.autograd.functional.jacobian(
        lambda p: qntk_single_sample(p, input_data), params, create_graph=False
    )

    jacobian = jacobian.view(input_data.size(0), -1)
    qntk_matrix = jacobian @ jacobian.T

    eigenvalues = torch.linalg.eigvalsh(qntk_matrix).real
    eig_max = eigenvalues.max().item()

    filtered_eigenvalues = eigenvalues[eigenvalues > 1e-10]
    if filtered_eigenvalues.numel() == 0:
        eig_min = 1e-12  # 최소 eigenvalue가 없을 경우 작은 수로 지정
    else:
        eig_min = filtered_eigenvalues.min().item()

    # eig_min = eigenvalues[eigenvalues > 1e-10].min().item()
    condition_number = eig_max / eig_min
    eig_entropy = -torch.sum(eigenvalues * torch.log(eigenvalues + 1e-10)).item()

    return {
        "qntk_matrix": qntk_matrix,
        "eig_max": eig_max,
        "eig_min": eig_min,
...
def rerun_good_bad(landscape_resuls_list, good_idx, bad_idx,
    ...
    for gidx in good_idx:
        ...
        NQE_results_g = [NQE(random_circuit, batch_size_for_NQE, iter_for_NQE, X_train, X_test, Y_train, Y_test) for _ in
                       range(num_of_trial)]
    ...
    for bidx in bad_idx:
        ...
        NQE_results_b = [NQE(random_circuit, batch_size_for_NQE, iter_for_NQE, X_train, X_test, Y_train, Y_test) for _ in
                       range(num_of_trial)]
